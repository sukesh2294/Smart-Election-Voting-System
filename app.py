...
from models import db,Vote, Voter, OTPVerification, Admin, Candidate, Election
...
def start_face_scan():
    data = request.get_json()
    user_id = data.get('aadhar_number', 'temp_user')
    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            if (len(face_session['faces_data']) < FRAMES_TOTAL and
                ...
                face_session['faces_data'].append(resized_img)
                face_detected = True
...
def save_face_data_to_db(user_id, faces_data):
    try:
        ...
        avg_face_data = np.mean(faces_array, axis=0)

        print("Saving face data for email:", repr(user_id))

        # ✅ Voter table mat use karo — use OTPVerification
        otp_record = OTPVerification.query.filter_by(email=user_id).order_by(OTPVerification.created_at.desc()).first()
        if otp_record:
            otp_record.temp_face_encoding = json.dumps(avg_face_data.tolist())
            db.session.commit()
            ...
            print("❌ OTPVerification not found with email:", repr(user_id))
            return False
...
def voter_login_verify_face():
    ...
    try:
        ...
        try:
            ...
            face_match = face_distance < 0.6  # Threshold for face matching
        except:
            # Fallback comparison
            face_match = True  # For demo purposes
...
def voter_dashboard():
    ...
    if not voter:
        print("Voter not found, clearing session")  # Debug log
        session.pop('voter_login', None)
        return redirect(url_for('voter_login_step1'))

    print(f"Rendering dashboard for voter: {voter.first_name}")  # Debug log
...
def cast_vote():
    ...
    try:
        ...
        # Here you would implement actual vote recording
...
def admin_dashboard():
    ...
    admin = db.session.get(Admin,session['admin_login']['admin_id'])
    return render_template('admin_dashboard.html', admin=admin,elections=elections)
